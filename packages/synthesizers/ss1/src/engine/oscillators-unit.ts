...
function buildUnisonPartialSpecs(
  ...
  const baseSpec =
    unisonBaseSpecs[prUnison as keyof typeof unisonBaseSpecs] ??
    unisonBaseSpecs[0];

  const numUnison = baseSpec.numUnison;
...
export function createOscillatorsUnit(
  ...
  return {
    ...
    update() {
      ...
      const numUnison = unisonPartialSpecs.length;
      const oscPartials = unisonManager.preservePartials(numUnison, playing);
      const waveform = getCustomWaveform(ac, { shape: pr[pk.shape] });
