export function createChorus2(ctx: AudioContext) {
  ...
  return {
    inputNode,
    outputNode,
    update(chorusLevel: number): void {
      wetGain.gain.value = chorusLevel * 0.6;
      lfoGainL.gain.value = chorusLevel * 0.005;
      lfoGainR.gain.value = chorusLevel * 0.005;
    },
    cleanup() {
      lfoL.stop();
      lfoR.stop();
